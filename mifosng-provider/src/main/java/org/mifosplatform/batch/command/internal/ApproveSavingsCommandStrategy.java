 ...
 */
@Component
public class ApproveSavingsCommandStrategy implements CommandStrategy {

    private final SavingsAccountsApiResource savingsAccountsApiResource;

    @Autowired
    public ApproveSavingsCommandStrategy(final SavingsAccountsApiResource savingsAccountsApiResource) {
    ...
    public BatchResponse execute(final BatchRequest request, @SuppressWarnings("unused") UriInfo uriInfo) {
        ...
        try {
            ...
            // Calls 'approve' function from 'SavingsApiResource' to approve a savings          
            responseBody = savingsAccountsApiResource.handleCommands(accountId, "approve", request.getBody());
