 ...
 */
@Component
public class ActivateSavingsCommandStrategy implements CommandStrategy {

    private final SavingsAccountsApiResource savingsAccountsApiResource;

    @Autowired
    public ActivateSavingsCommandStrategy(final SavingsAccountsApiResource savingsAccountsApiResource) {
    ...
    public BatchResponse execute(final BatchRequest request, @SuppressWarnings("unused") UriInfo uriInfo) {
        ...
        try {
            ...
            responseBody = savingsAccountsApiResource.handleCommands(accountId, "activate", request.getBody());
